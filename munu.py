...
if os.path.basename(sys.executable.lower()) in ["mayabatch.exe", "maya.exe"]:
    ...
    import ctypes
    #调用 c语言的dll文件msvcr100 中的函数_getmaxstdio
...
def analyze_maya(options, using_multi_task=0):
    ...
    #分析出来的工程目录有误 workspace.mel 无的话创建,有的话设置场景工程目录
    ...
    if os.path.exists(options["maya_file"]) and os.path.isfile(options["maya_file"]):
        try:
            ...
            elif options["user_id"] == 1857429 :
                pm.openFile(options["maya_file"],esn = 0, force=1, ignoreVersion=1, prompt=0, loadReferenceDepth="all")
            else:
...
#        else:
        ...
        cmd = "\"%s\" a \"%s\" \"%s\" -mx3 -ssw" % (RvOs.str_to_unicode(self.exe),RvOs.str_to_unicode(zip_file), RvOs.str_to_unicode(src))
...
class MayaTask(dict, RvOs, Zip7):
    ...
    def __init__(self, options):
        ...
        """获取 ini 文件内的信息 将信息加入全局变量 self.custom"""
        self.get_preference()
...
#                          "renderfile": self.scene_path.server_long_path,
        ...
        #设置特殊用户
        if  int(server_config["user_id"]) in [961743]:
            options["analyze_mode"] = "fast"
...
#            self["texture"] += [RvPath(i) for i in self.find_cache_files(i[0], i[1])]
        ...
        for node_i in pm.ls(type="cacheFile"):
            cache_path = node_i.cachePath.get()
            cache_name = node_i.cacheName.get()
            if cache_path and cache_path.strip() :
                #非盘符路径的表达式
                p = re.compile(r'^(?:[a-z][:][\\/]|[\\\\]|(?:\${?))', re.I)
                #判断路径是否为  盘符路径 如果不是 在前面添加工程目录路径
                if not p.findall(cache_path):
                    cache_path = os.path.join(pm.workspace(q=1, fullName=1), cache_path)
                #抓取 "xml", "mcc", "mcx", "bin", "mc" 后缀格式的缓存文件
                if cache_name and cache_name.strip():
                    if not cache_name.lower().endswith(".bin"):
                        self.caches.append(RvPath(os.path.join(cache_path, cache_name + ".xml")))
                        
                        
                    else :
                        cache_name = cache_name.rsplit(".", 2) + "*"

                    for i in ["xml", "mcc", "mcx", "bin", "mc"]:
                        # for a in  self.get_file_from_folder(cache_path, cache_name, i):
                        #     print a
                        self.caches += [RvPath(ii) for ii in self.get_file_from_folder(cache_path, cache_name, i)]
    ...
    def get_aiVolume(self):
        pprint.pprint("Get aiVolume node ...")
        textures = []
        nodes = [node_i for node_i in pm.ls(type="aiVolume")]
        for node_i in nodes:
            amin_attr = 1
            if node_i.hasAttr("filename"):
                file_path = node_i.attr("filename").get()
                if file_path :
                    textures += [RvPath(i) for i in self.file_images(file_path, amin_attr)]
        pprint.pprint("End aiVolume node ...")
        return list(set(textures))
    ...
    def get_texture(self):
        ...
        if "aiVolume" in register_node_types:
            print "Getting aiVolume  files..."
            self.textures += self.get_aiVolume()
    ...
    def get_layer_settings(self, render_layer=None):
        print "start get layers settins"
        render_settings = pm.PyNode("defaultRenderGlobals")
        if render_layer:
            try:
...
if __name__ == '__main__':
    ...
    print networks
    print all
    a = RvPath(r'D:\work\picture\timg.jpg')
    print(a)
...
#
    ...
    #     print i
    # cmd = r'"D:\\Program Files\\新建文件夹\\RenderBus4.0\\Rayvision\\RenderBus4.0\\module\\bin\\windows-32-msvc2012\\7z.exe" a "C:/Users/winroot/Desktop/f.rayvision" "C:/Users/winroot/Desktop/f.ma" -mx3 -ssw'
    # for line in RvOs.run_command(cmd):
    #     print line
